...
The first three are one lookup, and none of them makes a network call — fetching is left to the
caller, so each fetch keeps its own size cap and its own error:
